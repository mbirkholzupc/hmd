...
parser.add_argument('--img', default = 'random', 
...
seed(datetime.now())
if opt.img == 'random':
    all_img_files=sorted(list(Path(dataset_path+'train/img').glob('*')))
    all_img_files=[x.name for x in all_img_files]
    trn_indices=[int(x[5:13]) for x in all_img_files]
    rndidx=randint(0,len(trn_indices)-1)
    imagefilename=dataset_path+'train/img/BTFM_%08d.png' % rndidx
    opt.img=imagefilename
